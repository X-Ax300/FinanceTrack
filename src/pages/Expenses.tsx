  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className={`text-2xl font-bold ${textPrimary}`}>{t('Expenses')}</h1>
          <p className={`text-sm mt-1 ${textSecondary}`}>{filtered.length} {t('records')} - {t('Total')}: {formatCurrency(total)}</p>
        </div>
        ...
        <div className="overflow-x-auto">
            ...
            <thead>
              <tr className={`border-b ${theme === 'dark' ? 'border-gray-800' : 'border-gray-100'}`}>
                {['Name', 'Category', 'Amount', 'Method', 'Date', ''].map((h) => (
                  <th key={h} className={`px-4 py-3 text-left text-xs font-medium ${textSecondary} uppercase tracking-wider`}>{h ? t(h) : h}</th>
                ))}
              </tr>
            </thead>
                ...
                <tr key={e.id} className={`transition-colors ${theme === 'dark' ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium"
                      ...
                      {getCategoryIcon(e.category)} {t(CATEGORY_LABELS[e.category])}
                    </span>
                  </td>
                  <td className="px-4 py-3">
          ...
          {filtered.length === 0 && (
            <div className={`text-center py-12 ${textSecondary}`}>
              ...
              <p className="text-sm">{t('No expenses found')}</p>
            </div>
          )}
        </div>
      </Card>
      ...
      {/* Add/Edit Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? t('Edit Expense') : t('Add Expense')}>
        <div className="space-y-4">
          <Input label={t('Name')} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t('Coffee, Groceries...')} required />
          <div className="grid grid-cols-2 gap-3">
            <Select label={t('Category')} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as ExpenseCategory })}>
              {CATEGORIES.map((c) => <option key={c} value={c}>{t(CATEGORY_LABELS[c])}</option>)}
            </Select>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select label={t('Method')} value={form.method} onChange={(e) => setForm({ ...form, method: e.target.value as PaymentMethod })}>
              {METHODS.map((m) => <option key={m} value={m} className="capitalize">{t(m.charAt(0).toUpperCase() + m.slice(1))}</option>)}
            </Select>
            <Input label={t('Date')} type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} required />
          </div>
          <Input label={t('Description (optional)')} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder={t('Additional notes...')} />
          <div className="flex gap-3 pt-2">
            <Button variant="ghost" className="flex-1" onClick={() => setModalOpen(false)}>{t('Cancel')}</Button>
            <Button className="flex-1" onClick={handleSave} loading={saving}>{editId ? t('Save Changes') : t('Add Expense')}</Button>
          </div>
        </div>
      </Modal>

      {/* Delete confirmation */}
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title={t('Delete Expense')}>
